<?php
    ...
    $chefs = $con->prepare('SELECT user_id,firstName FROM user WHERE NOT EXISTS (SELECT * FROM booking WHERE user.user_id = booking.chef_id AND date=? AND time_slot=?) AND user_type="chef"');
